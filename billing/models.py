...
from accounts.models import UserProfile, GuestProfile

import stripe
stripe.api_key = "sk_test_w8E9YC55rUXiwjqsBuqb6Uka"
...
class BillingProfileManager(models.Manager):
    def new_or_get(self, request):
        ...
        if user.is_authenticated:
            ...
            guest_user = GuestProfile.objects.get(id=guest_user_id)
            obj, created = self.model.objects.get_or_create(email=guest_user.email)
        else:
            pass
        return obj, created
...
def billing_profile_pre_Save(sender, instance, *args, **kwargs):
    if not instance.customer_id and instance.email:
        customer = stripe.Customer.create(email=instance.email)
        instance.customer_id = customer.id

pre_save.connect(billing_profile_pre_Save, sender=BillingProfile)


def userprofile_post_save(sender, instance, created, *args, **kwargs):
    if created and instance.email:
        BillingProfile.objects.get_or_create(user=instance, email=instance.email)

post_save.connect(userprofile_post_save, sender=UserProfile)
...
post_save.connect(cart_post_save, sender=Card)


class ChargeManager(models.Manager):
    def do(self, billing_profile, order, card=None):
        card_obj = card
        if card_obj is None:
